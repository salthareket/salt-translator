...
document.addEventListener('DOMContentLoaded', function () {
    ...
    startBtn.addEventListener('click', () => {
        const lang = langSelect.value;
        if (!lang || untranslatedPosts.length === 0) return;
        ...
        function translateNext() {
          ...
          const postId = untranslatedPosts[completed].ID || untranslatedPosts[completed].id;
          ...
          .then(res => {
            if (!res.success) {
              console.error(`#${postId} hata:`, res.data);
            }
            completed++;
            // progress % hesapla
    ...
    startCronBtn.addEventListener('click', () => {
        ...
        fetch(ajaxurl, {
            ...
            body: new URLSearchParams({
                action: 'start_post_translation_queue',
                lang: lang,
                _ajax_nonce: saltTranslator.nonce
            })
        })
